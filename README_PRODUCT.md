...
```bash
# Test with sample emails (1000 emails, shows learning)
python test.py
...
```bash
python triageflow.py
# Then enter emails (one per line)
```
...
# Process email
result = triage.process_single_email("Customer can't reset password")
...
```bash
# View terminal charts
python visualize.py

# Run full test (1000 emails with tracking)
python test.py
```
...
This is production-ready. It works. It saves money. Try it.
